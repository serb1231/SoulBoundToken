...
async function main() {
    ...
    const contract = await ethers.getContractAt("MyToken", NATIONAL_ID_ADDR, user);

    // make a list of users to mint for
    const usersToMintFor = [
    ...
    for (const userAddress of usersToMintFor) {
        try {
            console.log("Minting token to address", userAddress);
            
            // safeMint is the function
            const mintTx = await contract.safeMint(userAddress);
            ...
            console.log(`   New Balance: ${balance.toString()} Token(s)`);

        } catch (error: any) {
            console.error("Mint Failed for one of the users:", error.shortMessage || error.message);
        }
    }
    ...
    for (const userAddress of usersToMintFor) {
        ...
        console.log(`\n--- Testing transfer for user: ${userAddress} ---`);
        try {
            ...
            if (Number(balance) == 0) {
                console.log("There are no tokens. Cannot test transfer.");
                return;
            }
            ...
            const tokenId = await contract.tokenOfOwnerByIndex(userAddress, balance - 1n);
            ...
            const transferTx = await contract.transferFrom(userAddress, randomReceiver, tokenId);
